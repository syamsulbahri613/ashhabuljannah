<div class="content">
    <div class="headContent">
        <h4>Laporan
            <?php foreach ($programbyid as $pbi) : ?>
                <?= $pbi['nama_program'] ?>
            <?php endforeach; ?>
    ...
    <form action="" method="post">
        <div class="shortLaporan">
            ...
            <input type="hidden" name="kd" value="<?= $pbi['kd_program'] ?>">

            <div class="btshort">
                <button type="submit" class="btn btn-primary">Short</button>
                <a href="<?php
                            if ($start === "" || $end === "") {
                                echo base_url('adminuser/pdfById/' . $pbi['kd_program']);
                            } else {
                                echo base_url('adminuser/pdfByDateById/' . $pbi['kd_program'] . '/' . $start . '/' . $end);
                            }
                            ?>" style="text-decoration: none;"><button class="btn btn-warning btn-block mt-2" type="button">Cetak PDF</button></a>
            </div>
        </div>
    </form>
    ...
    <div class="content2">
        ...
        <table class="table mt-5">
            ...
            <th class="table-dark">Nominal Donasi</th>
        ...
        <table class="table mt-5">
            ...
            foreach ($pengeluaran as $peng) : ?>
                <tr>
                    ...
                    <td><button class="btn btn-primary btn-block btn-sm" type="button" data-toggle="modal" data-target="#formModalP<?= $peng['kd_keluar'] ?>">Detail</button></td>
                </tr>
            <?php endforeach; ?>
        </table>
    </div>
